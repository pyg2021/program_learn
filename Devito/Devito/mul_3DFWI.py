...
model0 = Model(vp=v, origin=origin, shape=shape, spacing=spacing,
                  space_order=8, nbl=nbl, bcs="damp", grid = model1.grid)
...
src_coordinates[:, 0] = np.linspace(0, model1.domain_size[0], num=nsources)
...
rec_coordinates[:, 0] = np.linspace(spacing[0], model1.domain_size[0] - spacing[0], num=nreceivers)
rec_coordinates[:, 1] = 980.    
rec_coordinates[:, 1] = 20.# Receiver depth
...
if __name__ == "__main__":
    ...
    plt.figure()
    plot_image(model1.vp.data[model1.nbl:-model1.nbl, model1.nbl:-model1.nbl, model1.nbl:-model1.nbl][50], vmin=2.4, vmax=2.8, cmap="cividis")
    plt.savefig("/home/pengyaoguang/data/devito/FWI/test_result/v_update.png")
    # plot_image(vp[50], vmin=2.4, vmax=2.8, cmap="cividis")
