...
while left<len(d1) and right<len(d2):
    if d1[left]==d2[right]:
        rtm=segyfile.iline[segyfile.ilines[left]][::20,::20][:,:100]
        print(rtm.shape)
        v=segyfile_2.iline[segyfile_2.ilines[right]][101:2101][::20,::10][:,:100]
        print(v.shape)
        res.append(d1[left])
        # print(v)
        sio.savemat('/home/pengyaoguang/data/real_data/RTM{}'.format(number),{'RTM':rtm})
        sio.savemat('/home/pengyaoguang/data/real_data/v{}'.format(number),{'v':v/1000})
        left+=1
        right+=1
        number+=1
        break
        ...
        # plt.imshow(v.T)
        # plt.colorbar()
        # plt.savefig("/home/pengyaoguang/program_learn/2D/2.png")
        # plt.close()
        break
        # print(left,right)
        # break
