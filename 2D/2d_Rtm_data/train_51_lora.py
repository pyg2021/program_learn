#针对于三维的数据，通过将overtrust数据的井数据得到微调后的结果（并改变了对encodeing参数的学习率），数据量100，三种数据块共存,通过少量真实数据微调,井数据没有权重，10口井10个井数据
import torch
...
# loss_1=torch.nn.L1Loss()
loss_1=torch.nn.MSELoss()
train(model,train_loader_1,test_loader_1,10000,device,optimizer,scheduler,loss_1,save_number=51)
# test(model,train_loader_1,loss_1,device)
# test(model,train_loader_2,loss_1,device)
