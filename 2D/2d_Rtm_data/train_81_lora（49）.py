#针对于三维的数据，通过将overtrust数据的井数据得到微调后的结果（并改变了对encodeing参数的学习率），数据量400，三种数据块共存,通过少量真实数据微调,井数据没有权重，10口井10个井数据
import torch
        ...
        with torch.no_grad():
            ...
            if test_loss+a>=loss_number:
                ...
                if number>20:
                    ...
                    # ], lr=5e-1)
                    optimizer = torch.optim.AdamW([
                            {'params': [param for name, param in model.named_parameters() if ('A' not in name and 'B' not in name and 's' not in name)], 'lr': 1e-3},# 冻结原始权重
                            {'params': [param for name, param in model.named_parameters() if 'A' in name or 'B' in name or 's' in name], 'lr': 5e-3} # 优化LORA参数
                        ], lr=5e-1)
                    if state==1:
...
def apply_lora_to_unet(model, rank, downsample_factor=16):
    for name, module in list(model.named_modules()):
        if isinstance(module, nn.Conv2d):
            # 替换卷积层为LORAConv2d包装器
            setattr(model, name, LORAConv2d_new(module, rank, downsample_factor))
...
# model=torch.load("/home/pengyaoguang/data/2D_data/2D_result/modeltest9_80.pkl")
# model=nn.parallel.DataParallel(model)
...
# loss_1=torch.nn.L1Loss()
loss_1=torch.nn.MSELoss()
train(model,train_loader_1,test_loader_1,10000,device,optimizer,scheduler,loss_1,save_number=81)
# test(model,train_loader_1,loss_1,device)
# test(model,train_loader_2,loss_1,device)
