#针对于三维的数据，通过将overtrust数据的井数据得到微调后的结果（并改变了对encodeing参数的学习率），400口井微调
import torch
...
z=torch.zeros(x.shape[0],100,100).to(device)
for i in range(z.shape[0]):
    for _ in range(20):
        z[i][random.randint(0,99)]=1
    # pass
trian_number=y.shape[0]
train_data=data_utils.TensorDataset(torch.from_numpy(x).float(),torch.from_numpy(y).float(),z.float())
...
def train(model,train_loader,test_loader,epoch,device,optimizer,scheduler,loss_1,ewc=None, ewc_lambda=0.5,save_number=0):
    ...
    for epoch_i in range(epoch):
        epoch_loss=0
        model.train()
        for i,(x,y,z) in enumerate(train_loader_1):
            x=x.to(device)
            y=y.to(device)
            optimizer.zero_grad()
            y_1=model(x)
            ...
            loss=loss_1(z*y_1,z*y)+10*total_variation_loss(y_1)
            print(loss,10*total_variation_loss(y_1)/loss)
            if ewc is not None:
            ...
            if test_loss+a>=loss_number:
                ...
                if number>200:
                    ...
                    optimizer = torch.optim.AdamW([
                        {'params': [param for name, param in model.named_parameters() if ('A' not in name and 'B' not in name and 's' not in name) and ('down' in name or 'in_conv' in name)], 'lr': 1e-4},
...
# ], lr=5e-1)
optimizer = torch.optim.AdamW([
    {'params': [param for name, param in model.named_parameters() if ('A' not in name and 'B' not in name and 's' not in name) and ('down' in name or 'in_conv' in name or 'up' in name )], 'lr': 1e-4},
...
# loss_1=torch.nn.L1Loss()
loss_1=torch.nn.MSELoss()
train(model,train_loader_1,test_loader_1,10000,device,optimizer,scheduler,loss_1,save_number=75)
# test(model,train_loader_1,loss_1,device)
# test(model,train_loader_2,loss_1,device)
