            ...
            if test_loss+a>=loss_number:
                ...
                if number>50:
                    ...
                    optimizer = torch.optim.AdamW([
                        {'params': [param for name, param in model.named_parameters() if 'A' not in name and 'B' not in name and 's' not in name], 'lr': 1e-4},  # 冻结原始权重
                        {'params': [param for name, param in model.named_parameters() if 'A' in name or 'B' in name or 's' in name], 'lr': 1e-1} # 优化LORA参数
                    ], lr=1e-1)
                    loss_1=torch.nn.L1Loss()
                    # loss_number=float('inf')
...
train(model,train_loader_1,test_loader_1,10000,device,optimizer,scheduler,loss_1,save_number=14)
# test(model,train_loader_1,loss_1,device)
# test(model,train_loader_2,loss_1,device)
