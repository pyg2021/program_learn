...
# x=np.concatenate((x_1,x_2,x_3),axis=0)
# y=np.concatenate((y_1,y_2,y_3),axis=0)
x,y=DataLoad2(25000+260,25000+260)
x=x[::1000]
y=y[::1000]
...
def train(model,train_loader,test_loader,epoch,device,optimizer,scheduler,loss_1,ewc=None, ewc_lambda=0.5,save_number=0):
    ...
    for epoch_i in range(epoch):
        ...
        loss_all.append(epoch_loss/1000000)
        ...
        if epoch_i%2==0 and epoch_i>20:
            ...
            plt.figure()
            plt.plot(range(len(loss_all)-5),loss_all[5:],label="train")
            # plt.plot(range(len(test_loss_all)-50),test_loss_all[50:]/1000000,label="test")
            plt.xlabel("epoch")
            plt.ylabel("loss")
            plt.legend()
...
train(model,train_loader_1,test_loader_1,10000,device,optimizer,scheduler,loss_1,save_number=76)
# test(model,train_loader_1,loss_1,device)
# test(model,train_loader_2,loss_1,device)
