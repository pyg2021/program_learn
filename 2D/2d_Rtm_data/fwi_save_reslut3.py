...
plt.savefig("/home/pengyaoguang/program_learn/2D/2d_Rtm_data/fwi/start_3.png")
plt.close()
v = v_init.clone()
v.requires_grad_()
...
plt.savefig("/home/pengyaoguang/program_learn/2D/2d_Rtm_data/fwi/true_3.png")
plt.close()
...
def closure():
    ...
    # loss = 1e9 * loss_fn(out[-1], observed_data)+0.1*loss_fn(v[::10],v_true[::10])+0.2*total_variation_loss(v)
...
for epoch in range(n_epochs):
    ...
    if epoch%10==0:
        plt.figure()
        plt.imshow((v.cpu().detach().numpy().T),cmap='jet')
        plt.colorbar()
        plt.savefig("/home/pengyaoguang/program_learn/2D/2d_Rtm_data/fwi/update_3.png".format(epoch))
