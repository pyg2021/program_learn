            ...
            if test_loss+a>=loss_number:
                ...
                if number>20:
                    ...
                    optimizer = torch.optim.AdamW([
                        {'params': [param for name, param in model.named_parameters() if ('A' not in name and 'B' not in name and 's' not in name) and ('down' in name or 'in_conv' in name)], 'lr': 1e-3},
...
# loss_1=torch.nn.L1Loss()
loss_1=torch.nn.MSELoss()
train(model,train_loader_1,test_loader_1,10000,device,optimizer,scheduler,loss_1,save_number=84)
# test(model,train_loader_1,loss_1,device)
# test(model,train_loader_2,loss_1,device)
