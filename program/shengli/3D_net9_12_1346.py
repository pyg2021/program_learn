...
from Model3D_unt3_1346 import net
import os 
...
BatchSize=15
device="cuda"
x_1,y_1=DataLoad(25000+0,25000+100)
x_2,y_2=DataLoad(25000+100,25000+150)
x_3,y_3=DataLoad(25000+150,25000+199)
...
def train(model,train_loader,test_loader,epoch,device,optimizer,scheduler,loss_1,ewc=None, ewc_lambda=0.5,save_number=0):
    ...
    for epoch_i in range(epoch):
        ...
        if epoch_i%5==0 and epoch_i>=0:
            ...
            plt.colorbar()
            plt.savefig("/home/yaoguang/data/3D_net_result/v_updata9_{}.png".format(save_number))
            plt.close()
            sio.savemat("/home/yaoguang/data/3D_net_result/v_updata9_{}.mat".format(save_number),{"v":model(x).cpu().detach()[0,0]})
            torch.save(model.state_dict(),"/home/yaoguang/data/3D_net_model/modeltest9_{}.pkl".format(save_number))

            plt.figure()
            plt.imshow(y.cpu().detach()[0,0,50,:,:].T)
            plt.colorbar()
            plt.savefig("/home/yaoguang/data/3D_net_result/v_real9_{}.png".format(save_number))
            plt.close()
            ...
            plt.ylabel("loss")
            plt.legend()
            plt.savefig("/home/yaoguang/data/3D_net_result/history9_{}.png".format(save_number))
            plt.close()
...
# model.load_state_dict(torch.load("/home/yaoguang/data/3D_net_model/modeltest9_10.pkl"))
optimizer = torch.optim.AdamW(model.parameters(),lr=1e-3)
scheduler=torch.optim.lr_scheduler.StepLR(optimizer,step_size=400,gamma=0.5)
# loss_1=torch.nn.L1Loss()
loss_1=torch.nn.L1Loss()
train(model,train_loader_1,test_loader_1,4000,device,optimizer,scheduler,loss_1,save_number=12)
# test(model,train_loader_1,loss_1,device)
# test(model,train_loader_2,loss_1,device)