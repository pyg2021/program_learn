...
from DataLoad_well_2_1346 import DataLoad
...
def train(model,train_loader,test_loader,epoch,device,optimizer,scheduler,loss_1,ewc=None, ewc_lambda=0.5,save_number=0):
    ...
    for epoch_i in range(epoch):
        ...
        if epoch_i%5==0 and epoch_i>=0:
            ...
            plt.colorbar()
            plt.savefig("/home/yaoguang/data/3D_net_result/v_updata10_{}.png".format(save_number))
            plt.close()
            sio.savemat("/home/yaoguang/data/3D_net_result/v_updata10_{}.mat".format(save_number),{"v":model(x).cpu().detach()[0,0]})
            torch.save(model.state_dict(),"/home/yaoguang/data/3D_net_model/modeltest10_{}.pkl".format(save_number))

            plt.figure()
            plt.imshow(y.cpu().detach()[0,0,50,:,:].T)
            plt.colorbar()
            plt.savefig("/home/yaoguang/data/3D_net_result/v_real10_{}.png".format(save_number))
            plt.close()
            ...
            plt.ylabel("loss")
            plt.legend()
            plt.savefig("/home/yaoguang/data/3D_net_result/history10_{}.png".format(save_number))
            plt.close()
