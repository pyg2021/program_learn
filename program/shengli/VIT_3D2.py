...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool='cls', channels=3, dim_head=64, dropout=0., emb_dropout=0.):
        ...
        assert  image_size % patch_height ==0 and image_size % patch_width == 0 

        num_patches = (image_height // patch_height) * (image_width // patch_width)*(image_width // patch_width)
        patch_dim = channels * patch_height * patch_width*patch_width
        assert pool in {'cls', 'mean'}

        self.to_patch_embedding = nn.Sequential(
            Rearrange('b c (h p1) (w p2) (l p3) -> b (h w l) (p1 p2 p3 c)', p1=patch_height, p2=patch_width ,p3=patch_width),
        ...
        self.inverse_embedding = nn.Sequential(
            ...
            Rearrange('b d (p c) -> b p c d', p=patch_height*patch_width*patch_width),
            
        )
        self.mlp_head2 = nn.Sequential(
            nn.LayerNorm((num_patches+1)),
            nn.Linear((num_patches+1), num_patches),
            Rearrange('b p c d -> b c p d'),
        )
        n=32
        self.conv3d=nn.Sequential(
            nn.Conv3d(channels,n,3,1,1),
            nn.InstanceNorm3d(n),
            nn.ReLU(True),
            nn.Conv3d(n,1,3,1,1)
                                  )
    ...
    def forward(self, img):
        ...
        x = self.to_latent(x)     
        x=self.mlp_head2(x).reshape(-1,2,100,100,100)       
        x=self.conv3d(x)                                       # Identity (b, dim)
        return x.reshape(-1,1,100,100,100)
