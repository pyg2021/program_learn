...
x_1,y_1=DataLoad(15000,15000+0)
x,y=x_1,y_1
trian_number=y.shape[0]
train_data=data_utils.TensorDataset(torch.from_numpy(x).float(),torch.from_numpy(y).float())
...
loss_1=torch.nn.L1Loss()
train(model,train_loader,test_loader,2000,device,optimizer,scheduler,loss_1,save_number=12)
...
scheduler=torch.optim.lr_scheduler.StepLR(optimizer,step_size=300,gamma=0.6)
loss_1=torch.nn.L1Loss()
train(model,train_loader_2,test_loader_2,2000,device,optimizer,scheduler,loss_1,save_number=13,ewc=ewc, ewc_lambda=10000)
test(model,test_loader,loss_1,device)
test(model,test_loader_2,loss_1,device)