...
class ViT(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool='cls', channels=3, dim_head=64, dropout=0., emb_dropout=0.):
        ...
        self.inverse_embedding = nn.Sequential(
            ...
            Rearrange('b d (p c) -> b p (d c)', p=patch_height*patch_width),
            
        )
        self.mlp_head2 = nn.Sequential(
            nn.LayerNorm((num_patches+1)*channels),
            nn.Linear((num_patches+1)*channels, num_patches)
        )
    ...
    def forward(self, img):
        ...
        return self.mlp_head2(x).reshape(-1,1,100,10000)
