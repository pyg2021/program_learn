...
plt.imshow(v.T)
plt.colorbar()
plt.savefig("/home/pengyaoguang/data/FWI_test/FWI_smooth/v_init0.png")

plt.figure()
plt.imshow(v_real.T)
plt.colorbar()
plt.savefig("/home/pengyaoguang/data/FWI_test/FWI_smooth/v_real.png")
...
def closure():
        ...
        loss = 1e5*loss_fn((out[-1][:,:,:]-torch.mean(out[-1]))/math.sqrt(torch.var(out[-1])), observed_data[i:i+m,:,:])
        loss.backward()
...
for epoch in range(n_epochs):
    ...
    if epoch%20==0:
        ...
        plt.colorbar()
        plt.savefig("/home/pengyaoguang/data/FWI_test/FWI_smooth/{}.png".format(epoch))
        end=time.time()
